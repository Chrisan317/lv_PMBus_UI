...
lv_obj_t *status_label, *status_value;
lv_obj_t *fanspeed1_label, *fanspeed1_value;
lv_obj_t *pout_percentage, *pout_value, *pout_arc;
lv_obj_t *label_fanspd1, *label_fanspd2;
lv_obj_t *slider_fan1, *slider_fan2;
lv_obj_t *statusWord_label;
...
void slider_event_cb(lv_event_t *e) {
    ...
    if (slider == slider_fan1) {
        lv_label_set_text_fmt(label_fanspd1, "Fan 1 Duty: %d%%", value);
        fanCommand(0x3B, (uint8_t)value);
    } else if (slider == slider_fan2) {
        lv_label_set_text_fmt(label_fanspd2, "Fan 2 Duty: %d%%", value);
        fanCommand(0x3C, (uint8_t)value);
    }
}
...
{   
    ...
    lv_obj_add_style(status_label, &style, 0);
    lv_label_set_text(status_label, "Status:");
    lv_obj_align(status_label, LV_ALIGN_TOP_LEFT, 110, 140);
    
    status_value = lv_label_create(tab1);
    lv_obj_add_style(status_value, &style, 0);
    lv_label_set_text(status_value, "0x00");
    lv_obj_align(status_value, LV_ALIGN_TOP_LEFT, 190, 140);
    ...
    // Tab2 內容   
    //slider_fan1
    slider_fan1 = lv_slider_create(tab2);
    lv_obj_set_width(slider_fan1, 180);
    lv_slider_set_range(slider_fan1, 0, 100);
    lv_obj_align(slider_fan1, LV_ALIGN_TOP_MID, 0, 50);
    lv_obj_add_event_cb(slider_fan1, slider_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    label_fanspd1 = lv_label_create(tab2);
    //lv_label_set_text(label_fanspd1, "fan control");
    lv_label_set_text_fmt(label_fanspd1, "Fan 1 Duty: %d%%", lv_slider_get_value(slider_fan1));
    lv_obj_add_style(label_fanspd1, &style, 0);
    lv_obj_align(label_fanspd1, LV_ALIGN_TOP_MID, 0, 20);

    //slider_fan2
    slider_fan2 = lv_slider_create(tab2);
    lv_obj_set_width(slider_fan2, 180);
    lv_slider_set_range(slider_fan2, 0, 100);
    lv_obj_align(slider_fan2, LV_ALIGN_TOP_MID, 0, 130);
    lv_obj_add_event_cb(slider_fan2, slider_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    label_fanspd2 = lv_label_create(tab2);
    lv_label_set_text_fmt(label_fanspd2, "Fan 2 Duty: %d%%", lv_slider_get_value(slider_fan2));
    lv_obj_add_style(label_fanspd2, &style, 0);
    lv_obj_align(label_fanspd2, LV_ALIGN_TOP_MID, 0, 100);

    initPMBus();
    uint8_t fanConfig = getFanconfig();
    Serial.println(fanConfig);
    if (fanConfig == 0x88) {//0x88 = Fan2 available
    //Serial.println("PMBus response is 0x88. Displaying Slider 2.");
    // Clear the HIDDEN flag to make it visible
    lv_obj_clear_flag(slider_fan2, LV_OBJ_FLAG_HIDDEN);
    lv_obj_clear_flag(label_fanspd2, LV_OBJ_FLAG_HIDDEN);
    } else {
        //Serial.println("PMBus response is NOT 0x88. Hiding Slider 2.");
        // Add the HIDDEN flag to hide it
        lv_obj_add_flag(slider_fan2, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(label_fanspd2, LV_OBJ_FLAG_HIDDEN);
    }
   
    // Tab3 內容
    statusWord_label = lv_label_create(tab3);
    lv_label_set_text(statusWord_label, "status tab");
    lv_obj_add_style(statusWord_label, &style, 0);
    lv_obj_align(statusWord_label, LV_ALIGN_TOP_MID, 0, 10);
...
void updateUI(PMBusData_t* pmbus_data) {
    ...
    String statusText = "";
    String detailText = "";
    uint16_t status_word = pmbus_data->statusWord;
    ...
    //update Vin
    sprintf(buffer, "%.1fV", pmbus_data->vin);
    ...
    //Update Status
    sprintf(buffer, "0x%04x", pmbus_data->statusWord);
    lv_label_set_text(status_value, buffer);
    //lv_label_set_text(status_value, buffer);

    // --- 解讀各 STATUS 位元，這裡僅顯示文字，實際讀取詳細狀態在 PMBus 任務中完成 ---
    if (status_word & (1 << 15)) statusText += "VOUT_FAULT\n";
    if (status_word & (1 << 14)) statusText += "IOUT/POUT FAULT\n";
    if (status_word & (1 << 13)) statusText += "INPUT_FAULT\n";
    if (status_word & (1 << 12)) statusText += "MFR_SPECIFIC_FAULT\n";
    if (status_word & (1 << 11)) statusText += "DC_GOOD_FAULT\n";
    if (status_word & (1 << 10)) statusText += "FAN_FAULT\n";
    if (status_word & (1 << 9))  statusText += "OTHER_FAULT\n";
    if (status_word & (1 << 8))  statusText += "UNKNOWN_FAULT\n";
    if (status_word & (1 << 7))  statusText += "BUSY\n";
    if (status_word & (1 << 6))  statusText += "PS_OFF\n";
    if (status_word & (1 << 5))  statusText += "VOUT_OVP\n";
    if (status_word & (1 << 4))  statusText += "IOUT_OCP\n";
    if (status_word & (1 << 3))  statusText += "VIN_UV_FAULT\n";
    if (status_word & (1 << 2))  statusText += "TEMPERATURE_FAULT\n";
    if (status_word & (1 << 1))  statusText += "CML\n";
    if (status_word & (1 << 0))  statusText += "OTHER_FAULT\n";

    lv_label_set_text(statusWord_label, statusText.c_str());
