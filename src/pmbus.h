...
#define READ_TEMP1              0x8D
#define READ_TEMP2              0x8E
...
// PMBus掃描範圍
#define PMBUS_SCAN_START          0x58
#define PMBUS_SCAN_END            0x5f
...
typedef struct {
    ...
    float current;      // A
    float power;        // W
    float pomax;         //Pout Max
    float pin;
    float vin;
    float iin;
    float temp1;
    float temp2;  // °C
    uint16_t fanSpeed;  // RPM
    uint16_t fanSpeed2;  // RPM
    uint16_t statusWord; // 16位狀態字
} PMBusData_t;
...
float readVoltage();
uint8_t scan();
float readL11(uint8_t);
float readL16(uint8_t,uint8_t);
